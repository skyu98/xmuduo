    ...
    httpCallback_(detail::defaultHttpCallback)
{
  server_.setConnectionCallback(
      boost::bind(&HttpServer::onConnection, this, _1));    // 这里直接指定两个函数为自己的成员函数了
                           ...
                           Timestamp receiveTime)
{
  ...
  if (!detail::parseRequest(buf, context, receiveTime))  // 解析协议并且保存
  {
...
void HttpServer::onRequest(const TcpConnectionPtr& conn, const HttpRequest& req)
{
  ...
  HttpResponse response(close);     // 传入传出参数
  httpCallback_(req, &response);    // 这里交给用户处理，得到response
  Buffer buf;   
  response.appendToBuffer(&buf);
  conn->send(&buf);         // 发送回去
  if (response.closeConnection())      // 如果是短连接，则关闭这边的写
  {
    conn->shutdown();
  }
}
